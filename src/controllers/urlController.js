...
const urlShorten = async function (req, res) {
  try {
   ...
   console.log( await GET_ASYNC(`${longUrl}`))

    if (Object.keys(req.body).length==0 || Object.keys(req.body).length>1)
      return res.status(400).send({ status: false, message: "Bad Request, body need only one key value pair of long url" });
    if (!longUrl)
    ...
    const result = {
      ...
      shortUrl,
      urlCode,
    };//instead of using longUrl:longUrl,... this is ES6 feature
...
const redirect = async function (req, res) {
  try {
    ...
    if (!shortId.isValid(urlCode))
      return res
        .status(400)
        .send({ status: false, message: "not a valid URL code in params" });
